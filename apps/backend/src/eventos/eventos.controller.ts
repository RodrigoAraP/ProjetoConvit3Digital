...
export class EventosController {
  ...
  async salvarEvento(@Body() evento: Evento) {
    ...
    if (eventoCadastrado && eventoCadastrado.id !== evento.id)
      throw new HttpException('Ja existe um evento com esse alias', 400)
  ...
  async acessarEvento(@Body() dados: { id: string, senha: string }) {
    ...
    if (!evento) throw new HttpException('Senha nao encontrado', 400)
    if (evento.senha !== dados.senha) throw new HttpException('Senha nao corresponde ao evento', 400)
  ...
  async buscarEvento(@Param('idOuAlias') idOuAlias: string) {
    ...
    if (Id.valido(idOuAlias)) evento = await this.repo.buscarPorId(idOuAlias, true)
    else evento = await this.repo.buscarPorAlias(idOuAlias, true)
